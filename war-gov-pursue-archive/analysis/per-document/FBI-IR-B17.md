# FBI-IR-B17 — FBI Photo B17 / Фото ФБР B17

## Metadata / Метаданные

- **Source code / Код источника:** `FBI-IR-B17`
- **Agency / Агентство:** FBI (collected); imagery derived from a U.S. military system; submitted to AARO
- **Incident date / Дата инцидента:** Late 2025 (HUD timestamp `12/31/99 18:20:48` is **incorrect** — system date/time was not set; per metadata; OCR rendered `:` as `.`)
...
- **Slug / Slug:** `fbi-photo-b17`
- **Raw file / Исходный файл:** `raw/pdf/fbi-photo-b17.pdf`
- **Transcript / Транскрипт:** `transcripts/fbi-photo-b17.txt` (bogus timestamp `12/31/99 18.20.48` only — HUD numerals not OCR-recovered; image-only PDF)
- **Source URL / URL источника:** https://www.war.gov/medialink/ufo/release_1/fbi-photo-b17.pdf
- **Cluster context / Контекст кластера:** Part of 32-photo FBI Western US 2025 IR cluster; B17 is a **Burst 2** frame (~18:20:48). Cluster-level analysis in `analysis/topical/region-fbi-western-us-2023-2025.md`.

## Summary / Резюме

Monochrome IR still showing **"two small, dark, circular objects"** near the center of the frame. Description is the most concise in the entire B-series — no quadrant qualifier, no background detail. Modal two-circle Burst-2 frame.

**RU:** Монохромный ИК-снимок с **"двумя маленькими, тёмными, круглыми объектами"** вблизи центра кадра. Описание — наиболее лаконичное во всей серии B: нет квалификатора квадранта, нет детали фона. Модальный кадр Серии 2 с двумя кругами.
...
| 3 | HUD timestamp `12/31/99 18:20:48` is **not** the actual capture time (RTC unset).<br/>**RU:** HUD-таймстамп `12/31/99 18:20:48` **не** является фактическим временем съёмки (RTC не установлен). | ⚠ PARTIAL (relative ordering within burst is usable; absolute time is bogus) | transcript + metadata disclaimer |
| 4 | Sensor format: monochrome IR with simplified central crosshair.<br/>**RU:** Формат сенсора: монохромный ИК с упрощённой центральной прицельной маркой. | ✅ CORROBORATED | metadata + transcript |

## Cross-references / Перекрёстные ссылки

**Within PURSUE corpus / В корпусе PURSUE:**
- Burst 2 sisters / Кадры-сёстры Серии 2: `FBI-IR-B13`, `FBI-IR-B14`, `FBI-IR-B15`, `FBI-IR-B16`, `FBI-IR-B18`, `FBI-IR-B19`, `FBI-IR-B20`, `FBI-IR-B21`, `FBI-IR-B22`, `FBI-IR-B23`, `FBI-IR-B24`
...
- **Distinguishing feature / Отличительная черта:** Shortest AARO description in the B-series; otherwise no unique features beyond burst-time placement. / Наиболее краткое описание AARO в серии B; в остальном нет уникальных черт помимо временного размещения в серии.
- **Burst-time placement / Временное размещение в серии:** Burst 2, t ≈ +1m55s after B19 (18:18:53 → 18:20:48).
