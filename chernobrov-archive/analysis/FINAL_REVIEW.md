...
Conventional-commits format used throughout (`feat:`, `fix:`, `docs:`, `chore:`). No author-attribution trailers, no "Generated by", no robot-emoji markers, no vendor attribution detected (`git log --all -100 | grep -iE "claude|co-author|generated by|anthropic"` returns only references to AI/CV pipeline content, not authorship metadata).
